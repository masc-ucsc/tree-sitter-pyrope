...
/*
 * Obsolete grammar constructs. The current grammar no longer produces these
 * nodes/tokens -- when/unless statements, the expression-level `:type` check,
 * and the ~&/~^/~| match-arm prefixes were all removed -- but prpfmt still
 * carries their now-unreachable handlers. Give them ids far outside the
 * parser's symbol range so those legacy `case` labels keep compiling while
 * never matching a real node. Remove once the dead handlers are deleted.
 */
enum {
  anon_sym_when = 0x10000,
  anon_sym_unless,
  sym_when_unless_cond,
